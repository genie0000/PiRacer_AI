...
# Project dashboard autonomous control (disabled until explicitly enabled on
# the Pi). target_speed_mps scales the trained pilot throttle from 0 to 1;
# it is a calibrated command, not encoder-measured speed.
DASHBOARD_CONTROL_ENABLED = False
DASHBOARD_CONTROL_HOST = "0.0.0.0"
DASHBOARD_CONTROL_PORT = 9200
DASHBOARD_CONTROL_TOKEN = None
DASHBOARD_CONTROL_STATE_PATH = os.path.join(CAR_PATH, "control_state.json")
DASHBOARD_HEARTBEAT_TIMEOUT_S = 1.5
DASHBOARD_MAX_SPEED_MPS = 0.50
DASHBOARD_MAX_THROTTLE = 0.20
DASHBOARD_STRAIGHT_STEERING = 0.0

# YDLidar X2 local safety gate. The official X2 settings are 115200 baud,
# sample rate 3K, single-channel=True, TYPE_TRIANGLE, and 4-8Hz scan rate.
LIDAR_SAFETY_ENABLED = False
LIDAR_PORT = "/dev/ydlidar"
LIDAR_STOP_DISTANCE_M = 0.10
LIDAR_FORWARD_HALF_ANGLE_DEG = 20.0
...
# Keep machine-specific calibration and dashboard secrets out of this tracked
# base configuration. ``myconfig.py`` is optional and overrides every setting
# above because it is imported last.
try:
    from myconfig import *  # noqa: F403
except ImportError:
    pass
