...
# Project dashboard autonomous control. Set ENABLED and TOKEN on the real
# PiRacer. target_speed_mps scales the model throttle; it is not measured
# wheel speed.
DASHBOARD_CONTROL_ENABLED = False
DASHBOARD_CONTROL_HOST = "0.0.0.0"
DASHBOARD_CONTROL_PORT = 9200
DASHBOARD_CONTROL_TOKEN = None
# Both processes are started from ~/mycar, so this is shared by the browser
# server and the DonkeyCar process without depending on config.py globals.
DASHBOARD_CONTROL_STATE_PATH = "control_state.json"
DASHBOARD_HEARTBEAT_TIMEOUT_S = 1.5
DASHBOARD_MAX_SPEED_MPS = 0.50
DASHBOARD_MAX_THROTTLE = 0.20
DASHBOARD_STRAIGHT_STEERING = 0.0

# YDLidar X2 USB local obstacle safety.
