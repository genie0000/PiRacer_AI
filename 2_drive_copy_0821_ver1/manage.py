...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    # Optional autonomous command input from the project dashboard. While a
    # valid START + heartbeat is present it selects DonkeyCar's Local Pilot
    # mode. STOP or a missed heartbeat forces user mode with zero controls,
    # so the pilot can no longer reach the actuator path.
    if getattr(cfg, 'DASHBOARD_CONTROL_ENABLED', False):
        from dashboard_control import ControlStateStore, DashboardControlPart, DashboardDriveControl
        ...
        V.add(
            ...
            outputs=['user/angle', 'user/throttle', 'user/mode', 'dashboard/autonomous_active', 'dashboard/pilot_throttle_scale', 'dashboard/max_throttle'],
        )
    else:
        # A dashboard-disabled car keeps its original DonkeyCar behaviour.
        V.add(Lambda(lambda: (True, 1.0, 1.0)), outputs=['dashboard/autonomous_active', 'dashboard/pilot_throttle_scale', 'dashboard/max_throttle'])
    ...
    class DriveMode:
        def run(self, mode,
                    ...
                    pilot_angle, pilot_throttle, dashboard_pilot_throttle_scale):
            if mode == 'user': 
                ...
                return pilot_angle, pilot_throttle * cfg.AI_THROTTLE_MULT * dashboard_pilot_throttle_scale
        
    V.add(DriveMode(), 
          inputs=['user/mode', 'user/angle', 'user/throttle',
                  'pilot/angle', 'pilot/throttle', 'dashboard/pilot_throttle_scale'],
    ...
    if getattr(cfg, 'DASHBOARD_CONTROL_ENABLED', False):
        V.add(DashboardDriveControl(),
            inputs=['angle', 'throttle', 'dashboard/autonomous_active', 'dashboard/max_throttle'],
            outputs=['angle', 'throttle'])
